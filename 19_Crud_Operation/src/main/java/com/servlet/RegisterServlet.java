...
public class RegisterServlet extends HttpServlet {
	protected void doPost(HttpServletRequest request,HttpServletResponse response) throws IOException,ServletException {
		...
		if(name.trim().isEmpty() || email.trim().isEmpty() || dob.trim().isEmpty() || address.trim().isEmpty() || qualification.trim().isEmpty()) {
		  ...
		  Student s1 = new Student(name,dob,address,qualification,email);
		 
		  StudentDao dao = new StudentDao(DBConnect.getConn());
		  
		  HttpSession session = request.getSession();
		  
		 boolean f = dao.addStudent(s1);
		 if(f) {
//			 session.setAttribute("succsmsg", "Student Data Inserted Sucessfully..."); // not working because jstl jar not added to this project
//			 response.sendRedirect("add_student.jsp"); // not working because jstl jar not added to this project
			 k.print("<h3 style = 'color:green'>Student Data Inserted Sucessfully</h3>");
			RequestDispatcher rd = request.getRequestDispatcher("/add_student.jsp");
			rd.include(request, response);
